...
async function renderCampaignBlocks(blocks: CampaignBlock[]) {
  ...
  for (const block of sortedBlocks) {
    ...
    if (block.type === "cta") {
      ...
      sections.push(`<section style="margin:0 0 28px;"><a href="${escapeHtml(url)}" style="display:inline-block;padding:12px 18px;border-radius:999px;background:#0f935d;color:#fff;font-weight:700;text-decoration:none;">${escapeHtml(label)}</a></section>`);
      continue;
    }
    ...
    if (block.type === "posts") {
      ...
      const items = posts
        .map((post) => {
          const translation = post.translations.find((entry) => entry.locale === post.locale) || post.translations[0];
      ...
      sections.push(`<section style="margin:0 0 28px;"><h2 style="margin:0 0 12px;font-size:22px;color:#0f172a;">Articulos destacados</h2>${items || "<p style=\"color:#475569;\">No hay articulos seleccionados.</p>"}</section>`);
      continue;
    }
  ...
  try {
    ...
    await db.emailLog.create({
      data: {
        ...
        payload: JSON.stringify({ subscriberId: params.subscriber.id, sendJobId: params.sendJobId || null }),
        sentAt: new Date(),
      },
    });
