...
function buildBlogTelegramMessage(post: Post & {
  translations: Array<{ locale: "ES" | "CA"; title: string; excerpt: string | null; content: string }>;
  categories: Array<{ category: { name: string } }>;
}) {
  const translation = post.translations.find((item) => item.locale === "ES") || post.translations[0];
  const title = translation?.title || post.slug;
  const excerptSource = translation?.excerpt || stripHtml(translation?.content || "");
  const excerpt = excerptSource.length > 220 ? `${excerptSource.slice(0, 217).trim()}...` : excerptSource;
...
async function processNewsletter(now: Date, dryRun: boolean, forceNewsletter = false): Promise<NewsletterSummary> {
  const config = getAutomationConfig();
  ...
  const [subscribers, posts, offers] = await Promise.all([
    ...
    db.offer.findMany({ where: { locale: "ES" }, orderBy: [{ updatedAt: "desc" }], take: config.newsletterMaxOffers }),
  ]);
    ...
    data: {
      ...
      blocks: {
        create: [
          ...
          { sortOrder: 3, type: "offers", content: JSON.stringify(offers.map((offer) => offer.id)) },
        ],
      },
    },
  });
...
async function processLeadStudySummary(now: Date, dryRun: boolean): Promise<LeadStudySummary> {
  ...
  const [leads, studies] = await Promise.all([
    ...
    db.studyRequest.findMany({ where: { createdAt: { gte: since } }, orderBy: [{ createdAt: "desc" }] }),
  ]);
...
export async function runDailyAutomation(options: AutomationOptions = {}): Promise<DailyAutomationSummary> {
  const now = new Date();
