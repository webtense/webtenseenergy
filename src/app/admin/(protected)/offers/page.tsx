    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-zinc-900/90 p-6">
        <div className="mb-6">
          ...
          <p className="mt-2 text-sm text-zinc-400">Pega una oferta, genera el borrador y publicalo tras revision. Debajo tienes visibilidad de catalogo y logs.</p>
        </div>
        <AdminDealsManager
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <article className="rounded-3xl border border-white/10 bg-zinc-900/90 p-6">
          ...
          <div className="mt-4 space-y-3">
            {offers.map((offer) => (
              <div key={offer.id} className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3">
                <p className="font-semibold text-zinc-100">{offer.title}</p>
                <p className="mt-1 text-sm text-zinc-400">{offer.price}{offer.oldPrice ? ` · antes ${offer.oldPrice}` : ""}</p>
                <p className="mt-2 text-xs text-zinc-500">{offer.category} · {offer.discount || "sin descuento"}</p>
              </div>
            ))}
          </div>
        </article>
          ...
          <div className="mt-4 space-y-3">
            {logs.map((log) => (
              <div key={log.id} className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3">
                ...
                <p className="mt-2 text-xs text-zinc-500">{log.status} · {new Intl.DateTimeFormat("es-ES", { dateStyle: "short", timeStyle: "short" }).format(log.createdAt)}</p>
              </div>
            ))}
          </div>
        </article>
