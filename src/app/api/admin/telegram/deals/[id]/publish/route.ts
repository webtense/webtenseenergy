...
export async function POST(request: Request, { params }: Props) {
  ...
  if (!botToken || !channelId) {
    return NextResponse.json(
      { message: 'Configura TELEGRAM_BOT_TOKEN y TELEGRAM_CHANNEL_ID en producción.' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    logger.error({ err: error }, 'Error publicando en Telegram');
    return NextResponse.json(
      { message: error instanceof Error ? error.message : 'No se pudo publicar en Telegram.' },
      { status: 500 }
    );
  }
}
