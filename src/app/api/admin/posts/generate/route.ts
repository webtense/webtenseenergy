...
function pickCategory(payload: GeneratePayload) {
  ...
  if (source.includes('ahorro') || source.includes('consumo') || source.includes('factura'))
    return 'Ahorro Energético';
  if (source.includes('review') || source.includes('reseña') || source.includes('comparativa'))
    return 'Reseñas';
  return 'Domótica';
}
...
function buildFallback(payload: GeneratePayload, locale: 'ES' | 'CA'): DraftPayload {
  ...
  const body =
    locale === 'CA'
      ? `<h2>Què volem resoldre</h2><p>${payload.objective?.trim() || 'Aclarir el context, les opcions disponibles i quan té sentit aplicar-les.'}</p><h2>Punts clau</h2><ul><li>${
          payload.points
            ?.split(/\n|,|;/)
            .find(Boolean)
            ?.trim() || 'Context i problema habitual'
        }</li><li>${payload.audience?.trim() || 'A qui va dirigit'}</li><li>${payload.cta?.trim() || 'Següent pas recomanat'}</li></ul><h2>Conclusió</h2><p>${payload.tone?.trim() || 'To proper i tècnic sense ser opac.'}</p>`
      : `<h2>Qué queremos resolver</h2><p>${payload.objective?.trim() || 'Aclarar el contexto, las opciones disponibles y cuándo tiene sentido aplicarlas.'}</p><h2>Puntos clave</h2><ul><li>${
          payload.points
            ?.split(/\n|,|;/)
            .find(Boolean)
            ?.trim() || 'Contexto y problema habitual'
        }</li><li>${payload.audience?.trim() || 'A quién va dirigido'}</li><li>${payload.cta?.trim() || 'Siguiente paso recomendado'}</li></ul><h2>Conclusión</h2><p>${payload.tone?.trim() || 'Tono cercano y técnico sin ser opaco.'}</p>`;
  ...
  try {
    ...
    if (!prompt) {
      return NextResponse.json(
        { message: 'Indica al menos la idea principal del artículo.' },
        { status: 400 }
      );
    }
...
- CTA: ${payload.cta?.trim() || 'Invitar a pedir estudio o contacto'}`;
    ...
    try {
      ...
      return NextResponse.json({
        draft: {
          ...
          seoDescription:
            parsed.seoDescription?.trim() || parsed.excerpt?.trim() || fallback.seoDescription,
    ...
    logger.error({ err: error }, 'Error generando post con IA');
