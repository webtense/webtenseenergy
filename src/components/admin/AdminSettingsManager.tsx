...
export function AdminSettingsManager({ initialFlags, initialSettings }: Props) {
  ...
  const settingValue = useMemo(
    () => (key: string, locale: "ES" | "CA") => settings.find((item) => item.key === `${key}:${locale}`)?.value || "",
    [settings],
  );
  ...
  const saveSettings = async () => {
    setBusy(true);
    setMessage("");
    const items = settings.map((item) => ({ key: item.key.replace(/:(ES|CA)$/, ""), locale: item.locale || "ES", value: item.value }));
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-zinc-900/90 p-6">
        ...
        <p className="mt-2 text-sm text-zinc-400">Panel central para comportamiento publico del sitio sin tocar codigo.</p>
      ...
      <section className="rounded-3xl border border-white/10 bg-zinc-900/90 p-6">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {flags.map((flag) => (
            <button key={flag.id} type="button" onClick={() => toggleFlag(flag)} className="flex items-center justify-between rounded-2xl border border-white/10 bg-zinc-950 px-4 py-4 text-left hover:border-primary-500">
              <div>
                <p className="font-semibold text-zinc-100">{flag.key}</p>
                <p className="mt-1 text-xs text-zinc-500">{flag.description || "Sin descripcion"}</p>
              </div>
              <span className={`rounded-full px-3 py-1 text-xs ${flag.enabled ? "bg-primary-600 text-white" : "bg-zinc-700 text-zinc-200"}`}>{flag.enabled ? "ON" : "OFF"}</span>
            </button>
          ))}
        </div>
      </section>
      ...
      {groups.map((group) => (
        <section key={group.title} className="rounded-3xl border border-white/10 bg-zinc-900/90 p-6">
          <h3 className="text-xl font-semibold text-white">{group.title}</h3>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {group.items.map((item) => (
              <div key={item.key} className="space-y-3 rounded-2xl border border-white/10 bg-zinc-950 p-4 md:col-span-2">
                <p className="text-sm font-semibold text-zinc-100">{item.label}</p>
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    <label className="mb-2 block text-xs uppercase tracking-[0.16em] text-zinc-500">ES</label>
                    <textarea value={settingValue(item.key, "ES")} onChange={(event) => setSettingValue(item.key, "ES", event.target.value)} className="h-28 w-full rounded-2xl border border-white/10 bg-zinc-900 px-4 py-3 text-sm text-zinc-100 focus:border-primary-500 focus:outline-none" />
                  </div>
                  <div>
                    <label className="mb-2 block text-xs uppercase tracking-[0.16em] text-zinc-500">CA</label>
                    <textarea value={settingValue(item.key, "CA")} onChange={(event) => setSettingValue(item.key, "CA", event.target.value)} className="h-28 w-full rounded-2xl border border-white/10 bg-zinc-900 px-4 py-3 text-sm text-zinc-100 focus:border-primary-500 focus:outline-none" />
                  </div>
                </div>
              </div>
            ))}
          </div>
          <button type="button" onClick={saveSettings} disabled={busy} className="mt-5 rounded-2xl bg-primary-600 px-5 py-3 text-sm font-bold text-white hover:bg-primary-500 disabled:opacity-60">{busy ? "Guardando..." : "Guardar ajustes"}</button>
        </section>
      ))}
    </div>
  );
