...
function parseAudience(value: string | undefined): AudienceRules {
  ...
  try {
    ...
    return {
      ...
      activeWithinDays:
        typeof parsed.activeWithinDays === 'number' && parsed.activeWithinDays > 0
...
function ensureAudienceBlock(blocks: Draft['blocks'], nextAudience?: AudienceRules) {
  const audience =
    nextAudience || parseAudience(blocks.find((block) => block.type === 'audience')?.content);
  const payload = JSON.stringify(audience);
  const existing = blocks.find((block) => block.type === 'audience');

  if (existing) {
    return blocks.map((block) =>
      block.type === 'audience' ? { ...block, sortOrder: 0, content: payload } : block
    );
  }
  ...
  return {
    ...
    blocks: ensureAudienceBlock([
      ...
      { sortOrder: 3, type: 'offers', content: '[]' },
      {
        sortOrder: 4,
        type: 'cta',
        content: '{"label":"Abrir web","url":"https://webtenseenergy.com"}',
      },
    ]),
  };
}
...
export function AdminNewsletterManager({
  ...
  const [draft, setDraft] = useState<Draft>(() =>
    initialCampaigns[0]
      ? {
          ...
          scheduledFor: initialCampaigns[0].scheduledFor
            ? initialCampaigns[0].scheduledFor.slice(0, 16)
            : '',
          blocks: ensureAudienceBlock(
            initialCampaigns[0].blocks.map((block) => ({
              id: block.id,
  ...
  const sourceOptions = useMemo(
    () =>
      Array.from(new Set(initialSubscribers.map((subscriber) => subscriber.source))).sort(
        (left, right) => left.localeCompare(right, 'es')
      ),
    [initialSubscribers]
  );
  const estimatedRecipients = useMemo(
    () => estimateRecipients(initialSubscribers, audience),
    [initialSubscribers, audience]
  );
  ...
  const updateAudience = (nextAudience: AudienceRules) => {
    setDraft((current) => ({
      ...current,
      blocks: ensureAudienceBlock(current.blocks, nextAudience),
    }));
  };
  ...
  const saveCampaign = async () => {
    ...
    const response = await fetch(
      draft.id ? `/api/admin/newsletter/campaigns/${draft.id}` : '/api/admin/newsletter/campaigns',
      {
    ...
    setCampaigns((current) => {
      ...
      const next = exists
        ? current.map((campaign) => (campaign.id === normalized.id ? normalized : campaign))
        : [normalized, ...current];
  ...
  const sendTest = async () => {
    ...
    setMessage(
      response.ok
        ...
        : payload.message || 'No se pudo enviar la prueba.'
    );
    setBusy(false);
  };
  ...
  const sendNow = async () => {
    ...
    setBusy(true);
    setMessage('');
    const response = await fetch(`/api/admin/newsletter/campaigns/${draft.id}/send`, {
      method: 'POST',
    });
    ...
    setMessage(
      response.ok
        ? `Envio lanzado. Entregados: ${payload.result?.delivered || 0}. Fallidos: ${payload.result?.failed || 0}.`
        : payload.message || 'No se pudo lanzar el envio.'
    );
    setBusy(false);
  };
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-zinc-900/90 p-6">
        ...
        <p className="mt-2 text-sm text-zinc-400">
          Editor funcional para campanas manuales con prueba, envio real y visibilidad de jobs,
          eventos y logs.
        </p>
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-zinc-500">Campanas</p>
          ...
          <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4">
            ...
            <p className="mt-2 text-3xl font-bold text-white">
              {initialSubscribers.filter((item) => item.isActive).length}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-zinc-500">Ultimos jobs</p>
        ...
        <div className="mt-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4">
          ...
          <p className="mt-2 text-sm text-zinc-200">
            ...
            {audience.sources.length
              ? ` · fuentes: ${audience.sources.join(', ')}`
              : ' · todas las fuentes'}
            {audience.requireConsent ? ' · solo consentidos' : ' · sin exigir consentimiento'}
            {audience.activeWithinDays
              ? ` · altas recientes (${audience.activeWithinDays} dias)`
              : ' · sin recorte de antiguedad'}
          </p>
          <p className="mt-2 text-sm font-semibold text-emerald-200">
            Estimacion rapida: {estimatedRecipients} destinatarios
          </p>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="rounded-3xl border border-white/10 bg-zinc-900/90 p-4">
          <button
            ...
            className="mb-4 w-full rounded-2xl bg-primary-600 px-4 py-3 text-sm font-bold text-white hover:bg-primary-500"
          >
            Nueva campana
          </button>
          <div className="space-y-2">
            {campaigns.map((campaign) => (
              <button
                ...
                className={`w-full rounded-2xl border px-4 py-3 text-left transition ${selectedId === campaign.id ? 'border-primary-500/50 bg-primary-500/10' : 'border-white/10 bg-zinc-950 hover:border-white/20'}`}
              >
          ...
          <article className="rounded-3xl border border-white/10 bg-zinc-900/90 p-6">
            <div className="grid gap-4 md:grid-cols-2">
              <input
                ...
                className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-zinc-100 placeholder:text-zinc-500 focus:border-primary-500 focus:outline-none"
              />
              <input
                value={draft.subject}
                onChange={(event) =>
              ...
              <input
                ...
                className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-zinc-100 placeholder:text-zinc-500 focus:border-primary-500 focus:outline-none md:col-span-2"
              />
              <select
                value={draft.locale}
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    locale: event.target.value as Draft['locale'],
                  }))
                }
                className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-zinc-100 focus:border-primary-500 focus:outline-none"
              >
              ...
              <select
                ...
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    scheduleType: (event.target.value || null) as Draft['scheduleType'],
                  }))
                }
                className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-zinc-100 focus:border-primary-500 focus:outline-none"
              >
                <option value="">Sin programacion</option>
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-zinc-950 p-4">
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateAudience({
                        ...audience,
                        locale: event.target.value as AudienceRules['locale'],
                      })
                    }
                ...
                <div>
                  ...
                  <select
                    value={audience.activeWithinDays?.toString() || ''}
                    onChange={(event) =>
                      updateAudience({
                        ...audience,
                        activeWithinDays: event.target.value ? Number(event.target.value) : null,
                      })
                    }
                    className="w-full rounded-2xl border border-white/10 bg-zinc-900 px-4 py-3 text-sm text-zinc-100 focus:border-primary-500 focus:outline-none"
                  >
                ...
                <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-zinc-900 px-4 py-3 text-sm text-zinc-200">
                  <input
                    type="checkbox"
                    checked={audience.requireConsent}
                    onChange={(event) =>
                      updateAudience({ ...audience, requireConsent: event.target.checked })
                    }
                  ...
                  {audience.sources.map((source) => (
                    <button
                      ...
                      onClick={() =>
                        updateAudience({
                          ...audience,
                          sources: audience.sources.filter((entry) => entry !== source),
                        })
                      }
                      className="rounded-full border border-primary-500/30 bg-primary-500/10 px-3 py-1 text-xs font-semibold text-primary-200"
                    >
                      {source} ×
                    </button>
                  ))}
                  ...
                  >
                    <div className="grid gap-3 md:grid-cols-[100px_160px_minmax(0,1fr)_80px]">
                      <input
                        ...
                        className="rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 focus:border-primary-500 focus:outline-none"
                      />
                      <select
                        value={block.type}
                        onChange={(event) =>
                        ...
                        className="rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 focus:border-primary-500 focus:outline-none"
                      >
                        {['intro', 'text', 'posts', 'offers', 'cta'].map((type) => (
                      ...
                      <textarea
                        ...
                        className="h-28 rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-500 focus:border-primary-500 focus:outline-none"
                      />
                      <button
                        type="button"
                        onClick={() =>
                        ...
                        className="rounded-xl border border-red-500/30 px-3 py-2 text-sm text-red-300 hover:border-red-400"
                      >
                        Quitar
                      </button>
                    </div>
              ...
              <button
                ...
                onClick={() =>
                  setDraft((current) => ({
                    ...
                    blocks: [
                      ...current.blocks,
                      { sortOrder: current.blocks.length + 1, type: 'text', content: '' },
                    ],
                  }))
                }
                className="rounded-2xl border border-white/10 px-4 py-3 text-sm font-semibold text-zinc-200 hover:border-primary-500 hover:text-primary-300"
              >
                Anadir bloque
              </button>
            </div>
            <div className="mt-6 flex flex-col gap-3 md:flex-row">
              <button
                ...
                disabled={busy}
                className="rounded-2xl bg-primary-600 px-5 py-3 text-sm font-bold text-white hover:bg-primary-500 disabled:opacity-60"
              >
                {busy ? 'Guardando...' : draft.id ? 'Guardar cambios' : 'Crear campana'}
              </button>
              <input
                value={testEmail}
                onChange={(event) => setTestEmail(event.target.value)}
                placeholder="email de prueba"
                className="flex-1 rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-zinc-100 placeholder:text-zinc-500 focus:border-primary-500 focus:outline-none"
              />
              <button
                type="button"
                onClick={sendTest}
                disabled={busy}
                className="rounded-2xl border border-white/10 px-5 py-3 text-sm font-semibold text-zinc-200 hover:border-white/20 disabled:opacity-60"
              >
                Enviar prueba
              </button>
              <button
                ...
                onClick={sendNow}
                disabled={busy}
                className="rounded-2xl border border-emerald-500/30 px-5 py-3 text-sm font-semibold text-emerald-300 hover:border-emerald-400 disabled:opacity-60"
              >
                Enviar ahora
              </button>
            </div>
              ...
              <div className="mt-4 space-y-3">
                {jobs.slice(0, 8).map((job) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-zinc-500">
                      {job.status} · {formatDate(job.runAt)}
                    </p>
            ...
            <section className="rounded-3xl border border-white/10 bg-zinc-900/90 p-6">
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-4">
                <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4">
                  ...
                  <p className="mt-2 text-lg font-bold text-white">{selectedCampaign.status}</p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4">
                  <p className="text-xs uppercase tracking-[0.16em] text-zinc-500">Programada</p>
                  <p className="mt-2 text-lg font-bold text-white">
                    {formatDate(selectedCampaign.scheduledFor)}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4">
                  <p className="text-xs uppercase tracking-[0.16em] text-zinc-500">Enviada</p>
                  <p className="mt-2 text-lg font-bold text-white">
                    {formatDate(selectedCampaign.sentAt)}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4">
                  <p className="text-xs uppercase tracking-[0.16em] text-zinc-500">Bloques</p>
                  <p className="mt-2 text-lg font-bold text-white">
                    {selectedCampaign.blocks.length}
                  </p>
                </div>
              </div>
            </section>
