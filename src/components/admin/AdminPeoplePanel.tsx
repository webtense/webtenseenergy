...
export function AdminPeoplePanel({ initialPeople }: Props) {
  ...
  const selectedPerson =
    filteredPeople.find((person) => person.id === selectedId) || filteredPeople[0] || null;
        ...
        <div className="mt-2 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-zinc-400">
              Consolida lead, estudio, newsletter y actividad para leer mejor a cada contacto real.
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-3 text-sm text-zinc-300">
            <span className="font-semibold text-white">{initialPeople.length}</span> personas
            detectadas
          </div>
        </div>
        <input
          ...
          {filteredPeople.map((person) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`rounded-full px-3 py-1 text-xs font-semibold ${person.subscriberStatus === 'ACTIVE' ? 'bg-emerald-500/20 text-emerald-200' : person.subscriberStatus === 'INACTIVE' ? 'bg-amber-500/20 text-amber-200' : 'bg-zinc-700 text-zinc-300'}`}
                >
              ...
              <p className="mt-3 text-xs text-zinc-500">
                Leads {person.leadCount} · Estudios {person.studyCount} · Ultima actividad{' '}
        ...
        {selectedPerson ? (
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-white/10 bg-zinc-950 p-4 text-sm text-zinc-300">
              <p>
                <strong>Telefonos:</strong> {selectedPerson.phones.join(', ') || '-'}
              </p>
              <p className="mt-2">
                <strong>Empresas:</strong> {selectedPerson.companies.join(', ') || '-'}
              </p>
              <p className="mt-2">
                <strong>Locales:</strong> {selectedPerson.locales.join(', ') || '-'}
              </p>
              <p className="mt-2">
                <strong>Consentimiento:</strong> {formatDate(selectedPerson.consentedAt)}
              </p>
              <p className="mt-2">
                <strong>Ultima actividad:</strong> {formatDate(selectedPerson.lastActivityAt)}
              </p>
            </div>
            <div>
