...
export function AdminPipelineManager({ initialLeads, initialStudies }: Props) {
  ...
  const updateStudyStatus = async (id: string, nextStatus: Study["status"]) => {
    ...
    setStudies((prev) => prev.map((study) => (study.id === id ? { ...study, status: nextStatus } : study)));
    ...
    <div className="space-y-8">
      <section className="rounded-2xl border border-white/10 bg-zinc-900 p-6">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-zinc-400">Solicitudes de contacto persistidas con estado comercial y notas.</p>
        </div>
        <div className="space-y-4">
            ...
            <div key={lead.id} className="rounded-xl border border-white/10 bg-zinc-950 p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-zinc-400">{lead.email}{lead.phone ? ` · ${lead.phone}` : ""}</p>
                  <p className="text-xs text-zinc-500">{lead.subject || "Sin asunto"} · {lead.source} · {new Date(lead.createdAt).toLocaleString("es-ES")}</p>
                </div>
                <select
                  value={lead.status}
              ...
              <textarea
                ...
                onChange={(event) => setLeadNotes((prev) => ({ ...prev, [lead.id]: event.target.value }))}
              ...
              {lead.notes.length > 0 && (
                <div className="mt-3 space-y-2">
                  {lead.notes.slice(0, 3).map((note) => (
                    <div key={note.id} className="rounded-lg border border-white/5 bg-black/20 px-3 py-2 text-xs text-zinc-300">
                      <p>{note.body}</p>
                      <p className="mt-1 text-zinc-500">{note.adminUser?.username || "admin"} · {new Date(note.createdAt).toLocaleString("es-ES")}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          {leads.length === 0 && <p className="text-sm text-zinc-500">Todavia no hay leads persistidos.</p>}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-white/10 bg-zinc-900 p-6">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-zinc-400">Solicitudes del wizard con estado de revision y datos de adjunto.</p>
        </div>
        <div className="space-y-4">
          {studies.map((study) => (
            <div key={study.id} className="rounded-xl border border-white/10 bg-zinc-950 p-4">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-zinc-400">{study.email}{study.phone ? ` · ${study.phone}` : ""}</p>
                  <p className="text-xs text-zinc-500">{study.company || "Sin compania"} · {study.method} · {study.fileName || "Sin adjunto"}</p>
                  <p className="text-xs text-zinc-500">{new Date(study.createdAt).toLocaleString("es-ES")}</p>
                </div>
                <select
                  value={study.status}
                  onChange={(event) => updateStudyStatus(study.id, event.target.value as Study["status"])}
                  className="rounded-xl border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100"
                >
                  <option value="NEW">NEW</option>
          ...
          {studies.length === 0 && <p className="text-sm text-zinc-500">Todavia no hay estudios persistidos.</p>}
        </div>
      </section>
