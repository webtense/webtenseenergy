  ...
  return (
    <div className="surface-panel grid gap-8 p-6 md:grid-cols-[1.05fr_0.95fr] md:p-8">
      <div>
        ...
        <h3 className="mt-4 font-heading text-3xl font-bold tracking-tight text-foreground">
          Calcula tu ahorro potencial
        </h3>
        <p className="mt-3 text-sm leading-7 text-foreground/70">
          Ajusta consumo, potencia y perfil de uso. Obtendrás una estimación orientativa y la mejor
          siguiente acción para estudiar tu factura.
        </p>

        <div className="mt-6 space-y-5">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-foreground/45">
              Tarifa actual
            </p>
          ...
          <RangeField
            ...
            display={`${inputs.consumo} kWh`}
            min={50}
            max={800}
            step={10}
            onChange={(value) => update('consumo', value)}
          />
          <RangeField
            label="Potencia contratada"
            value={inputs.potencia}
            display={`${inputs.potencia.toFixed(2)} kW`}
            min={2.3}
            max={15}
            step={0.05}
            onChange={(value) => update('potencia', value)}
          />
          <RangeField
            label="Precio energía"
            value={inputs.precio}
            display={`${inputs.precio.toFixed(2)} €/kWh`}
            min={0.08}
            max={0.35}
            step={0.01}
            onChange={(value) => update('precio', value)}
          />
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-foreground/60">
                  Actívalo si ya tienes o estás valorando placas.
                </p>
              </div>
              <button
                type="button"
              ...
              >
                <span
                  className={`block h-6 w-6 rounded-full bg-white transition ${inputs.solar ? 'translate-x-6' : 'translate-x-0'}`}
                ></span>
              </button>
            </div>
            {inputs.solar ? (
              <div className="mt-4">
                <RangeField
                  ...
                  value={inputs.solarKw}
                  display={`${inputs.solarKw.toFixed(1)} kWp`}
                  min={1}
                  max={10}
                  step={0.5}
                  onChange={(value) => update('solarKw', value)}
                />
              </div>
            ) : null}
          </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="surface-panel-soft p-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <ResultCard
              ...
              value={money(result.facturaOptimizada)}
              highlight
            />
        ...
        <div className="surface-panel-soft p-5">
          ...
          <div className="mt-2 h-2 rounded-full bg-zinc-200 dark:bg-white/10">
            <div
              ...
              style={{ width: `${result.pctPotencia}%` }}
            ></div>
          </div>
        ...
        <div className="space-y-3">
          {result.tips.map((tip) => (
            <div
              key={tip}
              className="rounded-2xl border border-primary-200 bg-primary-50/80 px-4 py-3 text-sm leading-6 text-primary-900 dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-100"
            >
              {tip}
            </div>
          ))}
...
function ResultCard({
  ...
  return (
    <div className="rounded-2xl border border-zinc-200/80 bg-white p-4 dark:border-white/10 dark:bg-white/5">
      ...
      <p
        className={`mt-2 text-2xl font-bold tracking-tight ${highlight ? 'text-primary-600 dark:text-primary-300' : 'text-foreground'}`}
      >
        {value}
      </p>
    </div>
