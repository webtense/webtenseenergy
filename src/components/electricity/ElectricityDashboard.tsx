...
export default function ElectricityDashboard() {
  ...
  const derived = useMemo(() => {
    ...
    const now = data.now;
    const status =
      now <= data.average * 0.9
        ? 'Buen momento para consumir'
        : now >= data.average * 1.15
          ? 'Conviene posponer cargas'
          : 'Precio intermedio';
  ...
  return (
    <div className="mx-auto max-w-6xl text-foreground">
      <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-3 text-sm leading-7 text-foreground/65">
            Fuente: {data.source || 'mercado regulado'}. Actualizado:{' '}
            {data.updatedAt ? new Date(data.updatedAt).toLocaleString('es-ES') : 'ahora'}.
          </p>
        </div>
        ...
        <label className="flex items-center gap-3 rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          >
            <span
              className={`block h-6 w-6 rounded-full bg-white transition ${includeTaxes ? 'translate-x-6' : 'translate-x-0'}`}
            ></span>
          </button>
        </label>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          label="Precio ahora"
          value={formatPrice(data.now)}
          detail={derived.status}
          highlight
        />
        <MetricCard
          label="Media diaria"
          value={formatPrice(data.average)}
          detail="Referencia del día"
        />
        <MetricCard
          label="Mejor tramo"
          value={formatPrice(data.min.price)}
          detail={data.min.time}
          positive
        />
        <MetricCard
          label="Peor tramo"
          value={formatPrice(data.max.price)}
          detail={data.max.time}
          negative
        />
      </div>
        ...
        <div className="surface-panel-soft p-6">
          <div className="mb-5 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-foreground/60">
                Barras relativas para localizar de un vistazo los mejores y peores momentos del día.
              </p>
            </div>
          </div>
          <div className="space-y-3">
            {data.hourly.map((entry) => {
              const ratio =
                data.max.price > 0
                  ? Math.max(10, Math.round((entry.price / data.max.price) * 100))
                  : 0;
              ...
              return (
                <div
                  key={entry.hour}
                  className="grid grid-cols-[6.8rem_1fr_6.5rem] items-center gap-3 text-sm"
                >
                  ...
                  <span
                    className={`text-right font-semibold ${isMin ? 'text-primary-700 dark:text-primary-300' : isMax ? 'text-red-700 dark:text-red-300' : 'text-foreground/75'}`}
                  >
          ...
          <div className="surface-panel-soft p-6">
            ...
            <div className="mt-5 space-y-3">
              {derived.cheapest.map((entry) => (
                <div
                  key={entry.hour}
                  className="rounded-2xl border border-primary-200 bg-primary-50/80 px-4 py-3 dark:border-primary-500/20 dark:bg-primary-500/10"
                >
          ...
          <div className="surface-panel-soft p-6">
            ...
            <div className="mt-5 space-y-3">
              {derived.expensive.map((entry) => (
                <div
                  key={entry.hour}
                  className="rounded-2xl border border-red-200 bg-red-50/80 px-4 py-3 dark:border-red-500/20 dark:bg-red-500/10"
                >
...
function MetricCard({
  ...
  return (
    <div className="surface-panel-soft p-5">
      ...
      <p
        className={`mt-3 text-2xl font-bold tracking-tight ${highlight ? 'text-primary-700 dark:text-primary-300' : positive ? 'text-primary-700 dark:text-primary-300' : negative ? 'text-red-700 dark:text-red-300' : 'text-foreground'}`}
      >
        {value}
      </p>
