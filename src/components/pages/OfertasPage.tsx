...
export function OfertasPage({ initialOffers, refreshedAt, source }: Props) {
  ...
  const filtered = useMemo(() => {
    const selected = categoria === "Todos" ? initialOffers : initialOffers.filter((offer) => offer.category === categoria);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-24">
      <SectionHero
        ...
        align="center"
        compact
        actions={<Link href="https://t.me/webtenseenergy" target="_blank" rel="noopener noreferrer" className="cta-primary">Unirme al canal de Telegram</Link>}
      />
      ...
      <section className="section-shell">
        <div className="section-inner grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
          {filtered.map((offer) => {
            const discount = offer.oldPrice > offer.price ? Math.round(((offer.oldPrice - offer.price) / offer.oldPrice) * 100) : 0;
            return (
              <div key={offer.id} className="surface-panel-soft flex flex-col overflow-hidden p-5 transition hover:-translate-y-1 hover:border-brand-300 dark:hover:border-brand-500/20">
                <div className="flex flex-wrap gap-2">
                  {discount > 0 ? <span className="rounded-full bg-red-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-red-700 dark:bg-red-500/10 dark:text-red-300">-{discount}%</span> : null}
                  <span className="rounded-full bg-zinc-100 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-foreground/65 dark:bg-white/10 dark:text-zinc-300">{offer.category}</span>
                  {offer.tag ? <span className="rounded-full bg-amber-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">{offer.tag}</span> : null}
                </div>
                <div className="mt-5 flex h-40 items-center justify-center rounded-[1.5rem] bg-[radial-gradient(circle_at_top_left,rgba(26,183,117,0.12),transparent_36%),radial-gradient(circle_at_bottom_right,rgba(59,118,246,0.16),transparent_34%)] text-6xl">{offer.icon}</div>
                <h3 className="mt-5 font-heading text-2xl font-bold tracking-tight text-foreground">{offer.title}</h3>
                ...
                <div className="mt-5 flex items-end justify-between gap-4">
                  <div>
                    {offer.oldPrice > offer.price ? <p className="text-sm text-foreground/40 line-through">{offer.oldPrice.toFixed(2)} €</p> : null}
                    <p className="text-3xl font-bold tracking-tight text-brand-600 dark:text-brand-300">{offer.price.toFixed(2)} €</p>
                  </div>
                </div>
                <a href={offer.url} target="_blank" rel="noopener noreferrer" className="cta-primary mt-6 w-full">Ver producto</a>
              </div>
            );
          })}
        </div>
        <div className="section-inner mt-10 space-y-3 text-center text-xs leading-6 text-foreground/50">
          <p>Fuente de catálogo: {source === "database" ? "ofertas gestionadas en backoffice" : source === "cache" ? "caché local regenerable" : "fallback editorial"}.</p>
          <p>Selección editorial con enlaces de afiliación. Priorizamos productos útiles para control, ahorro y domótica aplicada.</p>
        </div>
      </section>
    </div>
  );
