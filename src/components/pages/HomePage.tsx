                ...
                <div className="relative z-10 space-y-6">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-white/70">
                      Ahorro potencial medio cuando la estrategia parte de datos reales.
                    </p>
                  </div>
        ...
        <div className="section-inner">
          <div className="flex flex-wrap gap-3">
            <div className="metric-chip">
              <span className="h-2 w-2 rounded-full bg-primary-500"></span>Estudio gratuito con
              respuesta en 48h
            </div>
            <div className="metric-chip">
              <span className="h-2 w-2 rounded-full bg-brand-500"></span>Consultoría para empresa y
              hogar
            </div>
            <div className="metric-chip">
              <span className="h-2 w-2 rounded-full bg-amber-500"></span>Blog, ofertas y
              herramientas en una misma plataforma
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {pillars.map((pillar) => (
              <Link
                key={pillar.title}
                href={pillar.href}
                className="surface-panel-soft group p-7 transition hover:-translate-y-1 hover:border-primary-300 dark:hover:border-primary-500/20"
              >
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2">
            {tools.map((tool) => (
              <Link
                key={tool.title}
                href={tool.href}
                className="surface-panel-soft group p-7 transition hover:-translate-y-1 hover:border-brand-300 dark:hover:border-brand-500/20"
              >
                ...
                <div className="mt-6 text-sm font-semibold text-brand-600 transition group-hover:translate-x-1 dark:text-brand-300">
                  {tool.cta} →
                </div>
              </Link>
            ))}
        ...
        <div className="section-inner grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="surface-panel-soft p-8 md:p-10">
            ...
            <h2 className="section-title mt-5 text-foreground">
              Guías, comparativas y decisiones bien explicadas
            </h2>
            <p className="section-copy mt-4 max-w-2xl">
              El blog y el canal de ofertas funcionan como extensión del servicio: explicar bien,
              filtrar mejor y reducir el ruido que rodea a la energía, la domótica y las compras
              técnicas.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href={withBasePath(basePath, '/blog')} className="cta-secondary">
                Ir al blog
              </Link>
              <Link
          ...
          <div className="surface-panel-soft p-8 md:p-10">
            ...
            <div className="mt-5 space-y-4 text-sm text-foreground/70">
              <div className="rounded-2xl border border-zinc-200/80 bg-white p-4 dark:border-white/10 dark:bg-white/5">
                Optimizar tarifa y potencia sin papeleo innecesario
              </div>
              <div className="rounded-2xl border border-zinc-200/80 bg-white p-4 dark:border-white/10 dark:bg-white/5">
                Entender cuándo conviene mover consumos a valle
              </div>
              <div className="rounded-2xl border border-zinc-200/80 bg-white p-4 dark:border-white/10 dark:bg-white/5">
                Elegir domótica y monitorización con criterio, no por moda
              </div>
            </div>
          </div>
        </div>
