...
export function calculateSavings(inputs: CalculadoraInputs): CalculadoraResultado {
  ...
  if (potencia > potenciaIdeal + 0.5) {
    descPotencia =
      (potencia - potenciaIdeal) *
      PRECIO_POTENCIA_DIA *
      DIAS_MES *
      (1 + IMPUESTO_ELECTRICO) *
      (1 + IVA);
  ...
  if (perfil === 'nocturno') {
    tips.push(
      'Tu perfil nocturno encaja bien con tarifas donde el tramo valle marque la diferencia.'
    );
  } else if (perfil === 'diurno') {
    tips.push(
      'Si trabajas desde casa, conviene estudiar tarifas más estables y optimizar equipos en segundo plano.'
    );
  } else if (perfil === 'fin') {
    tips.push(
      'Si concentras consumo los fines de semana, merece la pena revisar discriminación horaria real.'
    );
  } else {
  ...
  if (tarifa === 'plana') {
    ...
    tips.push(
      'Las tarifas planas suelen esconder margen; merece la pena compararlas con consumo real.'
    );
  }
  ...
  if (solar && solarKw > 0) {
    ...
    tips.push(
      `Tus ${solarKw.toFixed(1)} kWp pueden reducir una parte importante del término variable.`
    );
  }
